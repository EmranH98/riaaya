...
function createSeedState() {
  ...
  return {
    ...
    settings: {
      ...
      activeDate: today,
      branch: "الفرع الرئيسي",
      language: localStorage.getItem("riaayaLanguage") || "ar"
    },
...
function currentLanguage() {
  return state?.settings?.language || localStorage.getItem("riaayaLanguage") || "ar";
}
  ...
  els.salarySlip.innerHTML = `
    <div class="salary-slip-heading">
      <div>
        ...
        <h3>${member?.name || ""}</h3>
        <p>${label.date}: ${new Date(`${state.settings.activeDate}T12:00:00`).toLocaleDateString(currentLanguage() === "en" ? "en-US" : "ar-JO")} | ${label.branch}: ${state.settings.branch || "الفرع الرئيسي"}</p>
      </div>
...
function updateEntryPreview() {
  ...
  if (!service || (!previewEntry.doctorId && !previewEntry.specialistId)) {
    els.entryPreview.textContent = currentLanguage() === "en"
      ? "Choose a service and at least one doctor or specialist to preview payouts before saving."
      : "اختر الخدمة والطبيب أو الأخصائي لعرض صيغة المستحقات قبل الحفظ.";
    return;
  }
...
function renderSalaries(entries) {
  if (!canViewSensitive()) {
    els.salaryTable.innerHTML = `<tr><td colspan="6">كشف الرواتب والنسب مخفي لهذا الحساب.</td></tr>`;
  ...
  if (!rows.length) {
    els.salaryTable.innerHTML = `<tr><td colspan="6">أضف موظفين وعمليات لإظهار الرواتب.</td></tr>`;
    renderSalarySlip(entries);
    return;
  }
  ...
  els.salaryTable.innerHTML = rows.map(row => `
    <tr>
      ...
      <td><button class="text-button" type="button" data-select-salary-slip="${row.member.id}">تفاصيل الكشف</button></td>
    </tr>
  `).join("");
  renderSalarySlip(entries);
}
...
if (els.languageSelect) {
  els.languageSelect.addEventListener("change", event => {
    ...
    localStorage.setItem("riaayaLanguage", event.target.value);
    saveState();
    render();
  });
}
...
els.entryForm.addEventListener("submit", event => {
  ...
  if (!doctorId && !specialistId) {
    alert(currentLanguage() === "en"
      ? "Choose at least one doctor or specialist before saving the operation."
      : "اختر طبيباً أو أخصائياً واحداً على الأقل قبل حفظ العملية.");
    return;
  }
  state.entries.push({
    id: nextId("entry"),
