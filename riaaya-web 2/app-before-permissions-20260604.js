...
function renderEntryTable(entries) {
  if (!entries.length) {
    els.entryTable.innerHTML = `<tr><td colspan="6">أضف أول عملية حتى تظهر في سجل اليوم.</td></tr>`;
    return;
  }
  ...
  els.entryTable.innerHTML = entries.slice().reverse().map(entry => {
    ...
    return `
      <tr>
        ...
        <td>${money(netAmount(entry))}</td>
        <td><span class="formula-pill">${payoutText}</span></td>
        <td>
    ...
    <div class="staff-card">
      <div>
        ...
        <div class="staff-meta">
          ...
          <span class="pill">التكلفة ${money(service.defaultCost)}</span>
          <span class="pill">${service.active === false ? "متوقفة" : "فعالة"}</span>
        </div>
      </div>
      <button class="icon-button danger" type="button" data-delete-service="${service.id}">حذف</button>
    </div>
  `).join("");
}
    ...
    return `
      <div class="staff-card inventory-card">
        <div>
          ...
          <p>المورد: ${supplier?.name || "غير محدد"} | التكلفة ${money(item.unitCost)} | آخر طلب ${lastOrder}</p>
        </div>
...
function renderPurchaseOrders() {
  ...
  if (!state.purchaseOrders.length) {
    els.purchaseOrderList.innerHTML = `<tr><td colspan="9">لا يوجد سجل طلبات بعد.</td></tr>`;
    return;
  }
