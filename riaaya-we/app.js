...
function createReceiptForVisit(entries, patient) {
  if (!entries.length) return;
  state.receipts = state.receipts || [];
  state.receipts.unshift(normalizeReceipt({
    id: nextId("receipt"),
    ...
    total: entries.reduce((sum, entry) => sum + netAmount(entry), 0),
...
function receiptInvoiceXml(receipt) {
  ...
  const invoiceLines = entries.map((entry, index) => {
    ...
    <cac:InvoiceLine>
      ...
      <cac:Item><cbc:Name>${xmlValue(serviceLabel(entry))}</cbc:Name></cac:Item>
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
  ...
  <cbc:Note>${xmlValue(`Income source sequence: ${incomeSourceSequence}`)}</cbc:Note>
  ...
  <cac:AccountingCustomerParty>
    <cac:Party><cac:PartyLegalEntity><cbc:RegistrationName>${xmlValue(receipt.patient)}</cbc:RegistrationName></cac:PartyLegalEntity></cac:Party>
  </cac:AccountingCustomerParty>
  <cac:LegalMonetaryTotal>
    <cbc:LineExtensionAmount currencyID="JOD">${receipt.total.toFixed(3)}</cbc:LineExtensionAmount>
    <cbc:TaxExclusiveAmount currencyID="JOD">${receipt.total.toFixed(3)}</cbc:TaxExclusiveAmount>
    <cbc:TaxInclusiveAmount currencyID="JOD">${receipt.total.toFixed(3)}</cbc:TaxInclusiveAmount>
    <cbc:PayableAmount currencyID="JOD">${receipt.total.toFixed(3)}</cbc:PayableAmount>
...
function renderReceipts() {
  ...
  els.receiptTable.innerHTML = receipts.map(receipt => `
    <tr>
      ...
      <td>${["draft", "ready"].includes(receipt.status) ? `<button class="text-button" type="button" data-submit-receipt="${receipt.id}">إرسال إلى JoFotara</button>` : "-"}</td>
    </tr>
  `).join("");
}
...
if (els.whatsappSettings) {
  els.whatsappSettings.addEventListener("submit", event => {
    event.preventDefault();
    ...
    state.integrations.whatsapp = {
      ...
      configured: Boolean(data.businessPhone.trim() && data.phoneNumberId.trim())
    };
    saveState();
    renderCommunications();
  });
...
if (els.smsSettings) {
  els.smsSettings.addEventListener("submit", event => {
    event.preventDefault();
    const data = Object.fromEntries(new FormData(els.smsSettings).entries());
    state.integrations.sms = {
      provider: data.provider,
      senderId: data.senderId.trim(),
      endpoint: data.endpoint.trim(),
      configured: Boolean(data.senderId.trim() && (data.provider !== "custom" || data.endpoint.trim()))
    };
    saveState();
    renderCommunications();
  });
