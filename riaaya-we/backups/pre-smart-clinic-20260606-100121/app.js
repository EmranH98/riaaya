...
const PERMISSION_FEATURES = [
  ...
  { id: "expenses_report", label: "User Can Access Expenses Report Page", category: "reports", views: ["reports"], sensitive: true },
  { id: "expenses_settings", label: "User Can Access Expenses Settings Page", category: "financial", views: ["reconcile"], sensitive: true },
  ...
  { id: "sms_groups", label: "User Can Access SMS Groups", category: "communication" },
...
const seedPatients = [
  { id: "patient-mariam", patientNumber: "1001", profileType: "patient", name: "مريم أحمد", phone: "079 410 1001", email: "mariam@example.com", gender: "female", nationality: "الأردن", city: "عمّان", category: "ليزر", notes: "تفضّل التواصل عبر واتساب", createdAt: dateOffset(-120) },
  { id: "patient-omar", patientNumber: "1002", profileType: "patient", name: "عمر يوسف", phone: "078 410 1002", email: "", gender: "male", nationality: "الأردن", city: "الزرقاء", category: "تجميل", notes: "", createdAt: dateOffset(-98) },
  { id: "patient-hiba", patientNumber: "1003", profileType: "patient", name: "هبة محمود", phone: "077 410 1003", email: "hiba@example.com", gender: "female", nationality: "الأردن", city: "عمّان", category: "أسنان", notes: "حساسية من البنسلين", createdAt: dateOffset(-84) },
  { id: "patient-yazan", patientNumber: "1004", profileType: "patient", name: "يزن حسن", phone: "079 410 1004", email: "", gender: "male", nationality: "الأردن", city: "إربد", category: "عناية", notes: "", createdAt: dateOffset(-65) },
  { id: "patient-lin", patientNumber: "1005", profileType: "patient", name: "لين خالد", phone: "079 123 4455", email: "", gender: "female", nationality: "الأردن", city: "عمّان", category: "ليزر", notes: "تأكيد المواعيد عبر واتساب", createdAt: dateOffset(-44) },
  { id: "patient-sami", patientNumber: "1006", profileType: "patient", name: "سامي ناصر", phone: "078 222 1000", email: "", gender: "male", nationality: "الأردن", city: "عمّان", category: "تجميل", notes: "", createdAt: dateOffset(-32) },
  { id: "patient-huda", patientNumber: "1007", profileType: "patient", name: "هدى إبراهيم", phone: "077 333 0909", email: "", gender: "female", nationality: "فلسطين", city: "عمّان", category: "عناية", notes: "", createdAt: dateOffset(-18) },
  { id: "patient-malik", patientNumber: "1008", profileType: "visitor", name: "مالك فادي", phone: "079 444 0808", email: "", gender: "male", nationality: "الأردن", city: "عمّان", category: "استشارة", notes: "زائر لم يتحول إلى ملف علاجي بعد", createdAt: dateOffset(-5) }
];
...
function renderAccountList() {
  ...
  els.accountTable.innerHTML = rows.map(account => {
    const canDelete = account.id !== "account-admin" && account.id !== state.currentAccountId;
    return `
      <tr>
...
function setActiveDate(dateString) {
  state.settings.activeDate = dateString || today;
  if (els.clinicForm) {
  ...
  els.entryTable.innerHTML = pagination.items.map(entry => {
    ...
    return `
      <tr>
        ...
        ${showSensitive ? `<td>${canDelete ? `<button class="icon-button danger" type="button" data-delete-entry="${entry.id}">حذف</button>` : "-"}</td>` : ""}
      </tr>
    `;
  }).join("");
}
...
function renderPatientFile() {
  ...
  const operationRows = canUseFeature("patient_history") && operations.length
    ...
    `).join("")
    : `<tr><td colspan="5">${canUseFeature("patient_history") ? "لا توجد عمليات في هذا الملف." : "لا توجد صلاحية لعرض سجل العمليات."}</td></tr>`;
  ...
  els.patientFile.innerHTML = `
    ...
    <div class="patient-history-section">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>التاريخ</th><th>الخدمة</th><th>الحالة</th><th>الدفع</th><th>الصافي</th></tr></thead>
          <tbody>${operationRows}</tbody>
        </table>
      </div>
    </div>
...
function renderBookingCalendar() {
  ...
  els.bookingCalendar.innerHTML = calendarDaysForMonth(activeDate).map(day => {
    ...
    return `
      <button class="${dayClasses}" type="button" data-calendar-date="${day.date}" aria-label="${displayDate(day.date)}">
        <span class="calendar-day-head">
          <strong>${day.dayNumber}</strong>
          ${dayBookings.length ? `<small>${dayBookings.length}</small>` : ""}
        </span>
        <span class="calendar-day-bookings">
          ${chips || `<span class="calendar-empty">${isEnglish ? "Available" : "متاح"}</span>`}
          ${more}
        </span>
      </button>
    `;
...
function bookingScheduleColumns() {
  ...
  const members = state.staff
    ...
    .filter(member => !account.ownEntriesOnly || !account.staffId || member.id === account.staffId)
    .map(member => ({
      id: member.id,
      label: member.name,
...
function reportSourceLabel(source) {
  const labels = currentLanguage() === "en"
    ? { operation: "Operation", booking: "Booking", patient: "Patient / Visitor", inventory: "Inventory", service: "Service", supplier: "Supplier", staff: "Staff" }
    : { operation: "عملية", booking: "حجز", patient: "مريض / زائر", inventory: "مخزون", service: "خدمة", supplier: "مورد", staff: "موظف" };
...
function renderUniversalReport(items) {
  ...
  const body = items.length ? items.map(item => `
    <tr>
      ...
      <td>${canViewSensitive() && ["operation", "booking"].includes(item.source) ? money(item.value) : item.source === "patient" ? `${item.value} ${isEnglish ? "operations" : "عملية"}` : item.value || "-"}</td>
    </tr>
...
function renderReportVisuals(entries, bookings, patients, universalItems) {
...
function renderReports() {
  ...
  const financialReports = ["reconciliation", "byPatient", "perProcedure", "costs"];
  ...
  renderReportVisuals(allEntries, allBookings, allPatients, universalItems);
...
function updateEntryPreview() {
  ...
  const payoutTotal = payouts.reduce((sum, row) => sum + row.payout, 0);
  els.entryPreview.innerHTML = payouts.length
    ? `<span>صافي العملية <strong>${money(netAmount(previewEntry))}</strong></span><span>مستحقات الفريق <strong>${money(payoutTotal)}</strong></span>`
