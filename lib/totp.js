...
// Returns the matching HOTP counter within ±`window` steps, or -1. The caller
// can persist it to reject replays of an already-used code.
export function verifyTotpCounter(secretBase32, token, { window = 1, atMs = Date.now(), stepSeconds = 30 } = {}) {
  const clean = String(token || "").replace(/\D/g, "");
  if (clean.length !== 6) return -1;
  const secretBuffer = base32Decode(secretBase32);
  if (!secretBuffer.length) return -1;
  const counter = Math.floor(atMs / 1000 / stepSeconds);
  const candidate = Buffer.from(clean);
  for (let drift = -window; drift <= window; drift += 1) {
    const expected = Buffer.from(hotp(secretBuffer, counter + drift));
    if (expected.length === candidate.length && timingSafeEqual(expected, candidate)) return counter + drift;
  }
  return -1;
}

// Accepts codes within ±`window` steps to tolerate clock drift.
export function verifyTotp(secretBase32, token, options = {}) {
  return verifyTotpCounter(secretBase32, token, options) !== -1;
}
