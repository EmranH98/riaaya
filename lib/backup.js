...
// Patient photos live as files on the /data disk (not in the sqlite the DB
// backup captures). This gives them their own verified, prunable archive.
export function photosDir() {
  return process.env.RIAAYA_PHOTOS_DIR
    || (process.env.RIAAYA_DB_PATH ? join(dirname(process.env.RIAAYA_DB_PATH), "photos") : join(process.cwd(), "data", "photos"));
}

export function archivePhotos({ directory = backupDir(), retention = Number(process.env.RIAAYA_BACKUP_RETENTION || 30) } = {}) {
  const src = photosDir();
  if (!existsSync(src) || !readdirSync(src).length) return null;
  mkdirSync(directory, { recursive: true });
  const stamp = new Date().toISOString().replaceAll(":", "-").replaceAll(".", "-");
  const archivePath = join(directory, `riaaya-photos-${stamp}.tar.gz`);
  // System tar (present on Render/Linux and macOS) — a dependency-free archive.
  execFileSync("tar", ["-czf", archivePath, "-C", dirname(src), basename(src)]);
  if (!existsSync(archivePath) || statSync(archivePath).size === 0) {
    throw new Error("photo_archive_empty");
  }
  const keep = Math.max(Number(retention) || 30, 1);
  readdirSync(directory)
    .filter(name => name.startsWith("riaaya-photos-") && name.endsWith(".tar.gz"))
    .map(name => join(directory, name))
    .sort((a, b) => (a < b ? 1 : -1))
    .slice(keep)
    .forEach(path => rmSync(path));
  return { archivePath, size: statSync(archivePath).size };
}
